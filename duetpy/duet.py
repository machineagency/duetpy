#!/usr/bin/env python3
"""Driver for Controlling Duet 2 or 3 through socket or HTTP"""
...
def machine_is_homed(func):
    @wraps(func) # Hide this @property from normal code inspection utilities.
...
class Duet(Inpromptu):
    ...
    def __init__(self, address=LOCALHOST, debug=False, simulated=False, reset=False):
        ...
        # TODO: Do not assume 4 axes. Read them from the config.
    ...
    def connect(self):
        ...
        # Update local cache of machine configuration content.
        self._active_tool_index = None
        self.active_tool_index()
        # TODO: get the number of axes
        # TODO: get whether or not the machine is homed.
        self.axes_homed = json.loads(self.gcode("M409 K\"move.axes[].homed\""))["result"][:4]
        selfl.axis_count = len(self.axes_homed)
    ...
    def get_current_position():
        """Wait for any lingering moves to finish; then get the current position."""
        self.gcode(f"M400")
        response = self.gcode(f"M114")
    ...
    def reset(self):
        ...
        self._active_tool_index = None # reset the cached value.
        self.axes_homed = [False]*4 # reset starter value.
        self.disconnect()
        print("Reconnecting...")
        for i in range(10):
            time.sleep(1)
